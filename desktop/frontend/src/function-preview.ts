...
export class FunctionPreview {
  ...
  updateUI(entry: EntryPoint | null): Record<string, any> {
    ...
    // Don't replace the DOM while a slider is being dragged — the slider
    // element must stay alive so it can keep tracking the mouse position.
    if (!this.dragging) {
      this.render();
    }
  ...
  private makeParamInput(param: EntryPoint['params'][0]): HTMLElement {
    ...
    if (param.type === 'Bool') {
      ...
      toggleLabel.className = 'fn-preview-toggle';

      const input = document.createElement('input');
      input.type = 'checkbox';
      input.className = 'fn-preview-toggle-input';
      ...
      pill.className = 'fn-preview-toggle-pill';

      const text = document.createElement('span');
      text.className = 'fn-preview-toggle-text';
      ...
      seg.className = 'fn-preview-segmented';
      const cur = this.overrides[param.name] ?? param.default;
      for (const v of c.values) {
        const btn = document.createElement('button');
        btn.className = 'fn-preview-seg-btn';
        btn.textContent = String(v);
        if (cur != null && String(v) === String(cur)) btn.classList.add('active');
        btn.addEventListener('click', () => {
          seg.querySelectorAll('.fn-preview-seg-btn').forEach(b => b.classList.remove('active'));
          btn.classList.add('active');
          const val = parseFloat(btn.textContent!);
          this.overrides[param.name] = isNaN(val) ? btn.textContent! : val;
          this.applyOverrides();
        });
        seg.appendChild(btn);
      }
      ...
      const header = document.createElement('div');
      header.className = 'fn-preview-slider-header';
      ...
      numInput.className = 'fn-preview-slider-value';
      ...
      if (param.unit) {
        ...
        unitSpan.className = 'fn-preview-unit';
      ...
      const applyValue = (v: number) => {
        ...
        // Update the slider visually on every call — thumb must track the
        // mouse every frame regardless of how often the model re-renders.
        slider.value = String(clamped);
        numInput.value = slider.value;
        updateFill();
        this.overrides[param.name] = clamped;
        // Debounce the model re-render so the slider stays smooth.
      ...
      // Custom mouse drag tracked at document level — WKWebView loses the
      // native range grab when the mouse moves quickly off the element.
      slider.addEventListener('mousedown', (e) => {
        e.preventDefault();
        this.dragging = true;

        // Capture rect now — after applyOverrides() the panel re-renders and
        // this element is detached, making getBoundingClientRect() return zeros.
        ...
        const onMove = (me: MouseEvent) => applyValue(computeValue(me.clientX));
        const onUp = () => {
          document.removeEventListener('mousemove', onMove);
          document.removeEventListener('mouseup', onUp);
          this.dragging = false;
          // Flush the debounce and do a final render with the settled value.
      ...
      // Keep input listener for keyboard accessibility (arrow keys).
      ...
      bounds.className = 'fn-preview-slider-bounds';
      const minLabel = document.createElement('span');
      minLabel.textContent = param.unit ? `${min} ${param.unit}` : String(min);
      const maxLabel = document.createElement('span');
      maxLabel.textContent = param.unit ? `${max} ${param.unit}` : String(max);
      bounds.appendChild(minLabel);
      bounds.appendChild(maxLabel);
      ...
      row.className = 'fn-preview-input-row';
