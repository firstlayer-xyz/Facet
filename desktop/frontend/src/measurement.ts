...
export type SnapKind = 'vertex' | 'edgeMid' | 'edgeEnd' | 'edgePerp' | 'faceCentroid' | 'circleCenter' | 'circleEdge' | 'angleLock';

/**
 * Preset angles (degrees) for angle-lock snap. Measured from the pending
 * anchor's edge direction, in the half-plane containing the cursor. 0° aligns
 * with the edge, 180° anti-aligns; values in between cover common drafting
 * angles (30/45/60/90) and their supplements (120/135/150). */
const ANGLE_LOCK_PRESETS = [0, 30, 45, 60, 90, 120, 135, 150, 180] as const;

/** Capture zone around each preset — cursor direction within this many degrees
 *  of a preset locks to it. 5° keeps the closest presets (30/45 and 45/60) from
 *  stealing from each other (they're 15° apart, so ±5 is well clear of mid). */
const ANGLE_LOCK_CAPTURE_DEG = 5;
...
export interface Snap {
  ...
  /** For circleEdge: the center of the circle this rim point lies on. Pairs
   *  with a circleCenter anchor at the same hub to produce a radial dim. */
  hub?: Vec3;
  ...
  edge?: { a: Vec3; b: Vec3 };
  /** For circleCenter / circleEdge: welded points of the snapped loop, same
   *  space as `point`. Used to render the full-loop highlight. */
  loop?: Vec3[];
}
...
export function resolveSnap(args: {
  ...
  // --- 0. circleEdge: if the pending anchor is the center of a circle and the
  // cursor is near that same circle's rim, snap to the idealized point on the
  // circle nearest the hit. This is what turns an anchor-on-center into a
  // radial measurement: click center, drag to rim, commit ⇒ ∅diameter.
  if (args.pending && args.pending.kind === 'circleCenter' && args.pending.radius !== undefined && args.pending.axis) {
    // Find the matching circle in the cache by comparing centers in local space.
    let pendLocal: Vec3 = args.pending.point;
    if (invMW) {
      tmp.set(pendLocal[0], pendLocal[1], pendLocal[2]).applyMatrix4(invMW);
      pendLocal = [tmp.x, tmp.y, tmp.z];
    }
    for (const ce of args.cache.circularEdges) {
      if (length(sub(pendLocal, ce.center)) > Math.max(ce.radius * 1e-3, 1e-5)) continue;
      // Cursor in screen-space proximity to the loop?
      let minPx = Infinity;
      ...
      if (minPx > tol) break; // not near the rim — fall through to normal snap resolution
      // Project hit onto the circle plane and constrain to the circle radius.
      const fromC = sub(hitPointLocal, ce.center);
      const axialComp = dot(fromC, ce.axis);
      const planar = sub(fromC, scale(ce.axis, axialComp));
      const planarLen = length(planar);
      if (planarLen < 1e-9) break; // hit is on the circle axis — ambiguous rim point
      const u = scale(planar, 1 / planarLen);
      const rimLocal = add(ce.center, scale(u, ce.radius));
      return {
        kind: 'circleEdge',
        point: toWorld(rimLocal),
        radius: ce.radius,
        axis: ce.axis,
        hub: toWorld(ce.center),
  ...
  if (args.edgeLines && args.edgeLines.length > 0) {
    ...
    // --- 4a. angleLock: lock the dim line to a preset angle from the pending
    // anchor's edge, and snap to wherever that ray first crosses another edge.
    // Only active while placing the 2nd (or chain) pick, and only if the
    // anchor sits on an edge (face-centroid anchors are intentionally out).
    const pendingEdge = args.pending?.edge;
    if (pendingEdge && pendingLocal && args.edgeLines) {
      const anchorL = pendingLocal;
      // Anchor edge direction in local space. Both endpoints are transformed so
      // the direction is correct even with a non-rigid world matrix.
      const toLoc = (v: Vec3): Vec3 => {
        if (!invMW) return v;
        tmp.set(v[0], v[1], v[2]).applyMatrix4(invMW);
        return [tmp.x, tmp.y, tmp.z];
      };
      const eaL = toLoc(pendingEdge.a);
      const ebL = toLoc(pendingEdge.b);
      const edgeDir = normalize(sub(ebL, eaL));

      const hitDelta = sub(hitPointLocal, anchorL);
      const hitLen = length(hitDelta);
      if (hitLen > 1e-6 && length(edgeDir) > 0.5) {
        const H: Vec3 = scale(hitDelta, 1 / hitLen);
        const HdotE = Math.max(-1, Math.min(1, dot(H, edgeDir)));
        const perp = sub(H, scale(edgeDir, HdotE));
        const perpLen = length(perp);
        // Need a non-degenerate out-of-edge component to define the angle plane.
        if (perpLen > 1e-3) {
          const O: Vec3 = scale(perp, 1 / perpLen);
          const actualDeg = Math.acos(HdotE) * 180 / Math.PI;
          let bestDiff = Infinity;
          let preset = -1;
          for (const p of ANGLE_LOCK_PRESETS) {
            const d = Math.abs(actualDeg - p);
            if (d < bestDiff) { bestDiff = d; preset = p; }
          }
          if (bestDiff <= ANGLE_LOCK_CAPTURE_DEG) {
            const rad = preset * Math.PI / 180;
            const cE = Math.cos(rad), sO = Math.sin(rad);
            const dir: Vec3 = [
              cE * edgeDir[0] + sO * O[0],
              cE * edgeDir[1] + sO * O[1],
              cE * edgeDir[2] + sO * O[2],
            ];
            // Find the nearest forward edge whose closest-approach to the ray
            // is tight in 3D. Skew tolerance scales with distance so far-field
            // edges are still considered but near-miss skews are rejected.
            let bestRayT = Infinity;
            let bestPt: Vec3 | null = null;
            let bestAP: Vec3 | null = null;
            let bestBP: Vec3 | null = null;
            const nE = args.edgeLines!.length / 6;
            for (let e = 0; e < nE; e++) {
              const ax = args.edgeLines![e * 6],     ay = args.edgeLines![e * 6 + 1], az = args.edgeLines![e * 6 + 2];
              const bx = args.edgeLines![e * 6 + 3], by = args.edgeLines![e * 6 + 4], bz = args.edgeLines![e * 6 + 5];
              const aP: Vec3 = [ax, ay, az];
              const bP: Vec3 = [bx, by, bz];
              const ed: Vec3 = sub(bP, aP);
              const w0 = sub(anchorL, aP);
              const A = dot(dir, dir); // ≈ 1 (unit)
              const B = dot(dir, ed);
              const C = dot(ed, ed);
              if (C < 1e-18) continue;
              const Dv = dot(dir, w0);
              const Ev = dot(ed, w0);
              const denom = A * C - B * B;
              if (denom < 1e-10) continue; // ray parallel to this edge
              const rayT = (B * Ev - C * Dv) / denom;
              const segT = (A * Ev - B * Dv) / denom;
              if (rayT <= 1e-4) continue; // behind or at anchor
              if (segT < -1e-4 || segT > 1 + 1e-4) continue; // outside segment
              const rayPt: Vec3 = add(anchorL, scale(dir, rayT));
              const segPt: Vec3 = add(aP, scale(ed, segT));
              const skew = length(sub(rayPt, segPt));
              // Reject if the two lines don't actually meet: skew ≥ 5% of the
              // reach along the ray means the edge is well off-axis.
              if (skew > rayT * 0.05) continue;
              if (rayT < bestRayT) {
                bestRayT = rayT;
                bestPt = segPt;
                bestAP = aP;
            ...
            if (bestPt && bestAP && bestBP) {
              return {
                kind: 'angleLock',
                point: toWorld(bestPt),
                edge: { a: toWorld(bestAP), b: toWorld(bestBP) },
              };
            }
          }
        }
...
export function buildMeasurement(a: Snap, b: Snap): Measurement | null {
  // circleCenter + circleEdge on the same circle ⇒ radial. Accept either
  // order; the resolver produces (center, edge) in practice, but the
  // classifier stays order-independent.
  const center = a.kind === 'circleCenter' ? a : b.kind === 'circleCenter' ? b : null;
  const edge = a.kind === 'circleEdge' ? a : b.kind === 'circleEdge' ? b : null;
  if (center && edge && center.radius !== undefined && edge.hub &&
      length(sub(center.point, edge.hub)) <= Math.max(center.radius * 1e-3, 1e-5)) {
    return { kind: 'radial', c: center, radius: center.radius, diameter: center.radius * 2 };
  }
