...
func (e *Emitter) compElemList(el ast.CompElem) string {
	...
	case *ast.ForElem:
		// The ranges see the enclosing scope; the loop variables then shadow any
		// same-named let() binding while the body emits (see withLoopVarsShadowed).
		clauses := e.forClauses(n.Iters)
		return e.withLoopVarsShadowed(n.Iters, func() string {
			...
			// Body yields a list per iteration (each/nested-for/if-with-list) →
			// produce the list-of-lists and flatten it with fold. fold errors on
			// an EMPTY array, but OpenSCAD's comprehension over an empty source
			// yields [] — guard with a Size check (the inner comprehension is
			// pure, so emitting it twice only costs evaluation time).
			acc, elem := e.freshLoopVar(), e.freshLoopVar()
			inner := "for " + clauses + " { yield " + e.compElemList(n.Body) + " }"
			return "(Size(of: " + inner + ") == 0 ? [] : fold " + acc + ", " + elem + " (" + inner + ") { yield " + acc + " + " + elem + " })"
		})
	}
	panic(fmt.Sprintf("emit: unhandled list-comprehension element %T", el))
}

// withLoopVarsShadowed removes the for-clause loop variables from letScope while
// fn runs (the comprehension body emits), then restores them — so a loop variable
// shadows a same-named let() binding instead of being wrongly inlined as that
// binding's value.
func (e *Emitter) withLoopVarsShadowed(iters []ast.ForIter, fn func() string) string {
	if e.letScope == nil {
		return fn()
	}
	saved := make(map[string]string, len(iters))
