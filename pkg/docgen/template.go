...
func GenerateSite(site *DocSite, outDir string) error {
	...
	// Generate index page (no active guide)
	indexNav := buildNav(site, "")
	if err := writeHTMLPage(filepath.Join(outDir, "index.html"), "Facet Documentation", indexNav, buildIndexContent(site)); err != nil {
		return err
	}

	// Generate guide pages (each with its own nav showing that guide's sections)
	for _, g := range site.Guides {
		guideNav := buildNav(site, g.Slug)
		if err := writeHTMLPage(filepath.Join(outDir, g.Slug+".html"), g.Title+" - Facet", guideNav, g.HTML); err != nil {
			return err
		}
	}
	...
	apiNav := buildNav(site, "api")
	if err := writeHTMLPage(filepath.Join(outDir, "api", "index.html"), "API Reference - Facet", apiNav, buildAPIContent(site)); err != nil {
		return err
	}
...
func buildNav(site *DocSite, currentSlug string) string {
		...
		for _, g := range site.Guides {
			...
			if g.Slug == currentSlug {
				for _, sec := range g.Sections {
					fmt.Fprintf(&b, `<a href="%s.html#%s" style="padding-left:16px;font-size:13px">%s</a>`,
						g.Slug, sec.Slug, html.EscapeString(sec.Name))
				}
			}
		}
	}
	...
	for _, g := range site.APIGroups {
		...
		for _, s := range g.Sections {
			if s.Name != "" {
				fmt.Fprintf(&b, `<a href="api/index.html#%s" style="padding-left:16px;font-size:13px">%s</a>`, s.Slug, html.EscapeString(s.Name))
			}
		}
	}
...
func buildAPIContent(site *DocSite) string {
	...
	for _, group := range site.APIGroups {
		...
		for _, section := range group.Sections {
			if section.Name != "" {
				fmt.Fprintf(&b, `<h3 id="%s">%s</h3>`, section.Slug, html.EscapeString(section.Name))
			}
