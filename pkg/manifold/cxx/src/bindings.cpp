...
void facet_import_mesh(const char* path, FacetSolidRet* out, char** out_err) {
  *out_err = nullptr;
  ...
  wrap(new Manifold(Manifold(mesh_out).AsOriginal()), out);
}
...
#else  // FACET_WASM: mesh I/O is JS-side, not bound through Assimp

static char* dup_err(const char* msg) {
  size_t n = std::strlen(msg);
  char* out = static_cast<char*>(std::malloc(n + 1));
  if (out) std::memcpy(out, msg, n + 1);
  return out;
}
...
char* facet_export_mesh(ManifoldPtr* /*m*/, const char* /*path*/) {
  return dup_err("export_mesh: not available in wasm build (use JS-side encoder)");
}
#endif  // FACET_WASM
