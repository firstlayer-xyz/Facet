...
func (f *formatter) formatStmt(s parser.Stmt) {
	...
	case *parser.IfStmt:
		f.writeComments(s.Comments)
		f.writeIndent()
		// Preserve user intent: if the body was written on the same line as
		// the `if`, keep it inline when it fits. If the user wrote it on
...
// argsHaveComments reports whether any named arg carries comments.
func argsHaveComments(args []parser.Expr) bool {
	for _, a := range args {
		if na, ok := a.(*parser.NamedArg); ok && len(na.Comments) > 0 {
			return true
		}
	}
	return false
...
// opening '(' — pass 0 to suppress user-multi-line detection (used in
// measurement helpers that must always produce single-line output).
func (f *formatter) formatArgs(args []parser.Expr, callLine int) {
	if len(args) == 0 {
		return
	}
