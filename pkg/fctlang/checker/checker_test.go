...
fn Main() {
    ...
    var b = Sphere(r: 5 mm);
    return a + b - Cylinder(r: 3 mm, h: 10 mm);
}
`)
}
...
func TestCheckCylinderDefaultSegments(t *testing.T) {
	expectNoErrors(t, `fn Main() { return Cylinder(r: 5 mm, h: 10 mm); }`)
}
...
func TestCheckArgTypeMismatch(t *testing.T) {
	expectError(t, `
fn Main() {
    return Frustum(r1: 45 deg, r2: 10 mm, h: 5 mm);
}
`, "no matching overload for Frustum")
}
...
// TestCheckLambdaWithUserStructTypeMatch pins the fix for a checker bug
// where a function-parameter type like `fn(Vec3) Vec3` (Vec3 is a
// user-defined struct, not a builtin) compared unequal against a lambda's
// inferred `fn(Vec3) Vec3`. The stateless typeFromNameStr couldn't see
// structDecls, so the declared parameter resolved its inner Vec3 to
// typeUnknown while the lambda inference resolved it to typeStruct(Vec3),
// and typeCompatible returned false. Both sides displayed identically as
// "fn(Vec3) Vec3", making the error message spectacularly unhelpful.
func TestCheckLambdaWithUserStructTypeMatch(t *testing.T) {
	expectNoErrors(t, `
fn Transform(s Solid, f fn(Vec3) Vec3) Solid {
    return s;
}
fn Main() {
    return Transform(s: Cube(s: 10 mm), f: fn(p Vec3) Vec3 { return p });
}
`)
}

// TestCheckSolidWarpAcceptsLambda is the concrete case that surfaced the
// bug above: Solid.Warp's stdlib signature is fn(Vec3) Vec3 and a passing
// lambda must type-check.
func TestCheckSolidWarpAcceptsLambda(t *testing.T) {
	expectNoErrors(t, `
fn Main() {
    return Sphere(r: 10 mm).Warp(f: fn(p Vec3) Vec3 { return p })
}
`)
}
