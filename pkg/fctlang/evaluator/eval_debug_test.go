...
`
	...
	if result.Steps[last].Op != "Move" {
		t.Errorf("step %d: expected op Move, got %s", last, result.Steps[last].Op)
	}
	meshesLast := result.ResolveMeshes(last)
	if len(meshesLast) != 2 {
		t.Fatalf("step %d: expected 2 meshes (input + result), got %d", last, len(meshesLast))
	}
	roles := map[string]bool{}
	for _, m := range meshesLast {
		roles[m.Role] = true
		if m.Mesh == nil || m.Mesh.VertexCount == 0 {
			t.Errorf("step %d: mesh for role %s is empty", last, m.Role)
		}
	}
	if !roles["input"] || !roles["result"] {
		t.Errorf("step %d: expected roles input and result, got %v", last, roles)
	}
}
...
`
	...
	if result.Steps[last].Op != "Difference" {
		t.Errorf("step %d: expected op Difference, got %s", last, result.Steps[last].Op)
	}
	meshesLast := result.ResolveMeshes(last)
	if len(meshesLast) != 3 {
		t.Fatalf("step %d: expected 3 meshes (lhs + rhs + result), got %d", last, len(meshesLast))
	}
	roles := map[string]bool{}
	for _, m := range meshesLast {
		roles[m.Role] = true
	...
	if !roles["lhs"] || !roles["rhs"] || !roles["result"] {
		t.Errorf("step %d: expected roles lhs, rhs, result — got %v", last, roles)
	}
}
