...
// SectionName extracts the section title from the leading divider block in a
// comment slice, mirroring the stripping logic in DocComment. Returns "" if
// the comments do not begin with a divider-sandwiched title.
//
// Pattern matched:
//
//	# ----       ← opening divider (stripped)
//	# My Section ← title returned
//	# ----       ← closing divider (consumed)
func SectionName(comments []Comment) string {
	var lines []string
	for _, c := range comments {
		if c.IsDoc {
			lines = append(lines, c.Text)
	...
	for len(lines) > 0 {
		...
		// Non-divider line followed by a closing divider → sandwiched title.
		if len(lines) >= 2 && isDocDivider(strings.TrimSpace(lines[1])) {
...
type NamedArg struct {
	...
	Value    Expr
	Pos      Pos
	Comments []Comment // leading comments (lines before this arg) and trailing (end-of-line)
}
