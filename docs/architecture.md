...
When you need to add something new, write it in Go unless it has to call a C++ library. Validation, formatting, error prose, parameter munging, algorithmic code that manipulates data already owned by Go — all Go. Cross the boundary only to invoke Manifold / FreeType.

### Rules

1. **C++ wraps libraries, not logic.** A function that doesn't call into libmanifold or FreeType doesn't belong in `cxx/`. Pure math, parameter validation, data transformation, error formatting — Go.
...
4. **Memory ownership is unambiguous.** C++ allocates → C++ frees (via `facet_delete_*`). Go never holds a C pointer across function returns except through opaque handles (`Solid*`, `Sketch*`).

5. **Errors cross as codes; user prose is Go.** C signals failure with a null handle or a numeric status; Go formats the sentence the user sees. No C-allocated strings cross back to Go.
...
7. **File I/O in C++ only when the library owns the format.** FreeType owns font parsing, so glyph loading stays in C++. Mesh file formats (OBJ, STL, 3MF) are pure Go via the `meshio` package — C++ only builds a `Solid` from raw triangle data (`facet_solid_from_mesh`). Facet's own metadata (colors, face IDs, project files, JSON, logs) — Go.
