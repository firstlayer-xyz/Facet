...
//
// The two tests run sequentially (await), not concurrently: the shared harness
// calls process.exit(1) on failure, which would abort a still-running sibling,
// and two simultaneous browsers needlessly load the single dev server.
...
async function main() {
  await runTest('animation', async ({ page }) => {
    ...
    // Assert the stats are actually present before comparing — otherwise an
    // undefined volume makes `Math.abs(undefined - 1000) > 1` (NaN > 1) false,
    // so the test would pass vacuously if frame stats ever regressed away.
    if (!Number.isFinite(result.vol0) || !Number.isFinite(result.vol10)) {
      throw new Error(`frame stats missing volume: vol0=${result.vol0}, vol10=${result.vol10}`);
    }
  ...
  await runTest('animation-ui', async ({ page }) => {
    await page.waitForFunction(
